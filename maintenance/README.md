...
- `rule-router.json` — Work Type / Domain / Risk Signalから必要Owner Doc / Gateへ到達するPreflight Routeと、Interaction Completion Lifecycleを分けて保持するMachine-readable Routing正本
- `rule-router.schema.json` — Router構造Schema
- `loop-policy.schema.json` — Loop Engineering Product Contractで使うMachine-readable Policy Schema
- `loop-policy.example.json` — Default Branch direct-write / merge / deploy / secret accessを許可しないSafe Example Policy
...
Behavioral Rule本文は`docs/`のOwner Docを正本とします。`DEEP_SYSTEM_AUDIT.md`もNormative Ownerではなく、[`docs/14-continuous-improvement.md`](../docs/14-continuous-improvement.md) の手順を再現可能にする実行補助です。Loop EngineeringのProduct Contractは[`LOOP_ENGINEERING_REQUIREMENTS.md`](../LOOP_ENGINEERING_REQUIREMENTS.md)を参照し、Testing / Security / Project Management等のCommon Ruleは既存Ownerを維持します。
